...
const SponsorsSection = ({ sponsors, setSponsors }) => {
  const addSponsor = () => {
    setSponsors([
      ...sponsors,
      {
  ...
  return (
    <section className='space-y-6'>
      ...
      {sponsors.map(sponsor => (
        <div key={sponsor.id} className='rounded-3xl border p-6 space-y-5'>
          <div className='flex justify-between items-center'>
            <h3 className='font-bold'>{sponsor.name || 'New Sponsor'}</h3>
          ...
          <div>
            <label className='font-medium'>Sponsor Name</label>
          ...
          <div>
            ...
            <input
              ...
              accept='image/*'
              onChange={e =>
                updateSponsor(sponsor.id, 'logo', e.target.files[0])
              }
              className='mt-2 w-full rounded-xl border p-4'
            />
          </div>
